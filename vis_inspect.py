...
class vis_ins:
    def __init__(self, root, filepath):
        ...
        self.id_list = [f.split('_')[1] for f in self.files]
        ...
        self.l3.grid(row=3, column=0, pady=2)
        # add input boxes
    ...
    def next_pic(self):
        ...
        if self.i==len(self.i_list)-1:
            ...
            self.i += 1
            self.new_image = Image.open(self.filepath+str(self.files[self.i_list[self.i]]))
    ...
    def back_pic(self):
            ...
            if self.i==0:
                ...
                self.e2.delete(0, 'end')
                self.i -= 1
                self.new_image = Image.open(self.filepath+list(self.files[self.i_list[self.i]]))
                self.new_image = ImageTk.PhotoImage(self.new_image)
                self.imgl.config(image=self.new_image)
                self.imgl.image = self.new_image
                self.b2.configure(text="Next")
